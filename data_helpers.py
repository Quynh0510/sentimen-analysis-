...
def token (sentence) :
    ...
    api = data['api_news']
    r = requests.post(api, data = {"text" : sentence})
    return r.text
...
def predata(path):
    ...
    i = 0
    for comment in X:
        comment = stopword(tokenize(convert_Abbreviation(normalize_Text(clean_data(comment)))))
        X_re.append(comment)
...
def load_data_and_labels(positive_data_file, neutral_data_file, negative_data_file):
    ...
    neutral_examples = predata(neutral_data_file)
...
def stopword (sentence) :
    a = open('./data1/dict/stopwords.txt', 'r').read().split('\n')
...
def neg_word (sentence) :
    ...
    for i in result[0] :
        ...
        pos.append(i[1])
    # sentence = sentence.split()
    a = open('data1/dict/neg.txt', 'r').read().split()
    
    for i in range(len(sentence)) :
        if sentence[i] in a and pos[i] == 'RB' :
            k = True
            break
        else : k = False
    return k

# print(neg_word("máy vi tính không đẹp nhé"))
# print((tokenize('con cho kia, may la do mat day')))
